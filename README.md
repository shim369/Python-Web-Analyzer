...
* **保留判定（要確認）の自動生成検証**：接続失敗時（ページ数0）、階層数が計測不能なほど深い場合、および内部リンクを巡回し尽くさずにクロールが打ち切られた場合に、`RenewalEvaluator.decide()` が『要確認』を返す挙動と、`×`の根拠が1つでも確定していればそちらを優先する挙動の検証。これらの分岐は`evaluator.py`側に集約されており、`SiteScraperService`側にはこの判断ロジックを持たせていない。
